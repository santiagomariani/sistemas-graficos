        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            if ((textureColor.y > textureColor.x
            && textureColor.y > textureColor.z)
            || (textureColor.x > textureColor.z)) {
                position += normal*0.07;
            } else {
                position += normal*(2.0 + sin(uv.y*30.0*PI+time*30.0) + cos(uv.x*30.0*PI+time*30.0)) * 0.01;
            }
